...
const app = new App({
  ...
  customRoutes: [
    ...
    { path: '/health', method: ['GET'], handler: (req, res) => { res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ status: 'ok' })); } },
...
app.view('decision_modal_v2', async ({ ack, view, client }) => {
  try {
    await ack();
    app.view('decision_modal_new', async ({ ack, view, client }) => {
  // Copy the ENTIRE content from the decision_modal handler here
  try {
    await ack();
    ...
    console.log('=== NEW DECISION MODAL ===');
...
app.command('/decisions', async ({ command, ack, say }) => {
  await ack();
  app.command('/decision2', async ({ command, ack, client, say }) => {
  await ack();
  const decisionText = command.text.trim();
  if (!decisionText) { await say('❌ Provide text'); return; }
  try {
    await client.views.open({
      ...
      view: {
        type: 'modal',
        callback_id: 'decision_modal_new',
        private_metadata: JSON.stringify({ channel_id: command.channel_id, user_id: command.user_id, decision_text: decisionText }),
        ...
        blocks: [
          ...
          { type: 'divider' },
          { type: 'input', block_id: 'type_block', element: { type: 'static_select', action_id: 'type_select', options: [{ text: { type: 'plain_text', text: '📦 Product' }, value: 'product' }, { text: { type: 'plain_text', text: '🎨 UX' }, value: 'ux' }, { text: { type: 'plain_text', text: '⚙️ Technical' }, value: 'technical' }] }, label: { type: 'plain_text', text: 'Decision Type' } },
          { type: 'input', block_id: 'epic_block', optional: true, element: { type: 'plain_text_input', action_id: 'epic_input', placeholder: { type: 'plain_text', text: 'LOK-123' } }, label: { type: 'plain_text', text: 'Epic/Story Key' }, hint: { type: 'plain_text', text: 'Auto-fetches from Jira' } },
          { type: 'input', block_id: 'tags_block', optional: true, element: { type: 'plain_text_input', action_id: 'tags_input', placeholder: { type: 'plain_text', text: 'aem, integration' } }, label: { type: 'plain_text', text: 'Tags' } },
          { type: 'input', block_id: 'alternatives_block', optional: true, element: { type: 'plain_text_input', action_id: 'alternatives_input', multiline: true }, label: { type: 'plain_text', text: 'Alternatives' } },
          { type: 'input', block_id: 'jira_comment_block', optional: true, element: { type: 'checkboxes', action_id: 'jira_comment_checkbox', options: [{ text: { type: 'plain_text', text: '✅ Add this decision as a comment in Jira' }, value: 'yes' }] }, label: { type: 'plain_text', text: 'Jira Integration' } }
        ]
      }
    });
